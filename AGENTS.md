...
- `packages/shared` — código TypeScript puro sin dependencias de runtime: `kretzBarcode.ts`, `homeRoster.ts`, `catalogSearch.ts`. Vite (desktop renderer y mobile) consume el **source TS**. El proceso **main de Electron es CommonJS** (`tsc` → `dist-electron/`): no puede `require()` un archivo con `export`. Por eso `build:main` de desktop primero emite CJS en `packages/shared/dist-cjs/` (`tsconfig.cjs.json`) y el `exports.require` de shared apunta ahí. Nunca importar `@carniceria/shared` en `electron/` sin ese build CJS.
