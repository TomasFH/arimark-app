...
### B2 — Sync de ventas a Firestore al confirmar venta

**Contexto:**
Las ventas (`sales`) y sus ítems (`sale_items`) y pagos (`sale_payments`) solo viven en SQLite.
Ruta en Firestore: `licenses/{tenant_id}/sales/{saleId}`.
El documento de venta en Firestore debe incluir los ítems y pagos embebidos (subdocumentos o arrays en el mismo doc) para minimizar lecturas.

**Qué hacer:**
1. Leer `apps/desktop/electron/db/schema.ts` para ver `sales`, `saleItems`, `salePayments`.
2. Leer `apps/desktop/electron/ipc/sale.handler.ts` para encontrar el handler `CREATE_SALE`.
3. Leer `apps/desktop/electron/licensing/providerSync.ts` como referencia.
4. Crear `apps/desktop/electron/licensing/saleSync.ts` con:
   - `pushUnsyncedSales(tenantId: string): Promise<void>` — busca sales con `syncedAt=null`, construye el documento completo (sale + items + payments como arrays), lo empuja a Firestore, marca `syncedAt` en la tabla `sales`.
5. En `sale.handler.ts`, después de `CREATE_SALE` exitoso: llamar `pushUnsyncedSales(config.tenant_id).catch(...)`.
6. La tabla `sales` ya debería tener `syncedAt`. Verificar en schema; si no está, agregar migración.
7. Tests para el handler con mock del sync.
